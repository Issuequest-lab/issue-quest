...
if(a.translation==='machine')el.append(node('span','自動翻訳','tag'));if(a.translation==='unavailable')el.append(node('p','翻訳未取得。原文は詳細から確認できます。','notice'));
...
if(choosable){const label=node('label',undefined,'choose'),input=document.createElement('input');input.type='checkbox';input.checked=selected.has(a.id);input.addEventListener('change',()=>{if(input.checked){if(selected.size>=3){input.checked=false;$('#selectedCount').textContent='比較は3件まで。1件解除してください。';return;}selected.set(a.id,{s,a});}else selected.delete(a.id);updateSelection();});label.append(input,document.createTextNode('比較に選ぶ'));el.append(label);}return el;}
...
function render(){const area=$('#cards');area.replaceChildren();if(!payload)return;const kind=$('#kind').value,q=$('#search').value.trim().toLowerCase();let articleCount=0,successSources=0,failedSources=0;const groups=new Map();
for(const s of payload.sources){if(kind!=='all'&&s.kind!==kind)continue;const region=s.region||s.country||'その他';if(!groups.has(region))groups.set(region,[]);if(s.status!=='ok'){groups.get(region).push(unavailableCard(s));failedSources++;continue;}successSources++;let matched=0;for(const a of s.articles){if(q&&!`${a.titleJa||''} ${a.titleOriginal||''} ${s.name} ${region}`.toLowerCase().includes(q))continue;groups.get(region).push(card(s,a));articleCount++;matched++;}if(q&&matched===0){} }
